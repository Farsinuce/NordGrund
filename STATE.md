...
- **Four worlds exist, one origin, same coordinates in each.** Walk them at:
  ...
  `data\server-nordgrund` :25568 (that plus the §16 features), `data\server-dhm` :25569 (that plus
  work item 3: DHM 0.4 m terrain with the repair stages gated off).
  **#2, #3 and #4 have not been walked.**
- **Forks:** branch `nordgrund` in both, pushed. `Farsinuce/arnis` carries the §16 features, the
  ...
  `meld\arnis.exe` is our build of that branch (`--version` prints the commit; sha256
  `8c4a89152bbb0438014ce683102e83d057ec6ce82fa0b324efb5e0f546532c07`).
- **Work item 3 ran** (`research/dhm-provider-run-2026-09-03.md`): all four cells picked the
  `dhm_dk` provider, merged clean, boot clean. Against world #3, 62.6 % of ground columns are
  identical, 8.8 % move by 2+ blocks and 0.6 % by 5+; the movement clusters in the Aarhus Å valley
  and the railway cutting, which is what the 30 m Gaussian and the coastal pull were flattening.
  Real harbour depth is still invisible because the lock starts at 0 m — that is decisions.md O,
  and the first merge wins the height profile, so settle it before any production cell.
...
- **Awaiting the user:** the walk; decisions.md F, I, K, L, M, N, O.
- **Machine** (3 Sep 2026): Python 3.12.9, Rust 1.98.0 at `%USERPROFILE%\.cargo\bin` (not on the
  Bash tool's PATH), JDK 25.0.4, 16 threads, 32 GB, D: ~185 GB free. Meld: 4 workers × 4 threads.
...
- **Pins:** arnis base `78215bd` (v3.1.8), meld base `5c1353e` (v1.9.8), `louis-e/arnis` main was
  ...
  BBR, DAR for kommune 0751), plus the hand-verified `tools/dhm_sea_dk.txt`.
